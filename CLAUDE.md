...
**New AgentEvents** (`src/events.ts`, also carry optional `roleId`): `dispatch:start|event|end`, `cost:update`, `budget:alert`. **REST**: read — `/api/roles`, `/api/usage`, `/api/tools`, `/api/agents`; write — `POST /api/roles/:id` (persists a role's editable fields via `saveRole`). These must stay mirrored in `web/lib/types.ts` + `web/lib/store.ts` (and eventually `mobile/`).
...
**Web shell** (`web/`): a persistent left sidebar (`components/Sidebar.tsx`, desktop static column / mobile `Sheet` drawer) with four collapsible sections — **Sessions** (reuses `SessionList`), **Virtual company** (member rows: click → role-bound chat via `openRole()` which creates a `web-<roleId>-<rand>` session; ⚙ gear → role settings; plus workflow + projects `soon` placeholders — node-graph designer is a **deferred placeholder**), **Budget & spend** (`components/BudgetPanel.tsx`: total + budget bars, expandable keys breakdown), and **Settings**. The main pane switches on `store.view` (`welcome` ↔ `session` ↔ `settings`); `components/DispatchCard.tsx` renders delegated-coding progress inside the timeline. `components/Settings.tsx` edits each role's model / coding harness / action mode / tool allowlist and POSTs to `/api/roles/:id`; `SessionView` has a per-session action-mode toggle.
